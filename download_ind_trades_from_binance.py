...
base = 'eth'
quote = 'usdt'
continue_download = True  # поставить True, если хотим продолжить грузить данные. Например, год скачал, хочешь ещё год скачать
...
def get_individual_trades(asset_id_base: str = None, asset_id_quote: str = None, limit: int = 1000, from_id: int = None):
    # NB! У этого запроса вес = 5. Лимит - 1200/мин
    symbol_id = f"{asset_id_base}{asset_id_quote}".upper()
    url_cont = 'api/v3/historicalTrades'
    params = {
        'symbol': symbol_id,
        'limit': limit,
        'fromId': from_id
...
headers, trades = get_individual_trades(base, quote, limit=1000, from_id=download_from_id)
...
while True:
    ...
    for trade in trades:  # конвертируем timestamp в datetime
        trade['time'] = datetime.datetime.fromtimestamp(trade['time'] / 1000, tz=datetime.timezone.utc).replace(tzinfo=None)  # Делим на 1000, т.к. binance API выдаёт результат в милисекундах
    ...
    while True:
        try:
            headers, trades = get_individual_trades(base, quote, limit=1000, from_id=(oldest_id - 1000))
            used_limit = int(headers['x-mbx-used-weight-1m'])
        except:
